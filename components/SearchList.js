...
export default class SearchHome extends Component {
  constructor(props) {
    ...
    this.state = {
      search: null,
      data: FranceDictionary.FranceDictionary,
      fullData: FranceDictionary.FranceDictionary,
      loading: false,
      error: null,
      query: '',
    };
    ...
    return (
      <ListItem avatar>
        <Left>
          <Icon
            name="ios-search"
            style={{fontSize: 20, color: '#0208c4'}}
        ...
        <Body>
          <TouchableOpacity onPress={() => navigate('detailSearch', item)}>
            <Text style={{color: '#041700', fontSize: 14}}>
              {item.nameFr} {item.IPA}
            </Text>
            <Text>
              {item.data.map((item, index) => (
                <Text style={{color: '#4c6148', fontSize: 14}} key={index}>
                  ‣{item}{' '}
                </Text>
              ))}
            </Text>
  ...
  handleSearch = (text) => {
    ...
    const data = _.filter(this.state.fullData, (search) => {
      //lọc dữ liệu vào mảng fullData
      if (search.data[0].includes(formatedQuery)) {
        return true;
      }
      return false;
    });
